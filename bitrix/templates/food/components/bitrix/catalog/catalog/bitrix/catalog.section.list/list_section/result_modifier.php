...
//vong 1: get picture for section
if( CModule::IncludeModule("iblock") )
{
    foreach ($arResult['SECTIONS'] as $key => $arSection)
    {
        //echo 'search id=', $arSection['ID'], '<br>';
        $res_img = CIBlockElement::GetList(
            array(), //"rand"=>"ASC"
            array(
        ...
        if( $img_sec = $res_img->Fetch() )
        {
            //echo '<pre>'; var_dump($img_sec); echo '</pre>';
            $arResult['SECTIONS'][$key]['SPEC_IMG'] = CFile::GetPath($img_sec['PREVIEW_PICTURE']);
        }
    }
}
if( count($arResult['SECTIONS']) )
{
    define("VONG_COUNT_SECTION", count($arResult['SECTIONS']));
}
//end vong 1
?>