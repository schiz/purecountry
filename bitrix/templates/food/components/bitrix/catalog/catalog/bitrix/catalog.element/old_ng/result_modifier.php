...
if ($arResult['MODULES']['catalog'])
{
	...
	if ($boolSKU && !empty($arParams['OFFER_TREE_PROPS']))
	{
		...
		$rsProps = CIBlockProperty::GetList(
			...
			array('IBLOCK_ID' => $arSKU['IBLOCK_ID'], 'ACTIVE' => 'Y', 'MULTIPLE' => 'N')
		);
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		if ('' != $arParams['OFFER_ADD_PICT_PROP'] && isset($arOffer['PROPERTIES'][$arParams['OFFER_ADD_PICT_PROP']]))
		{
			if ('F' == $arOffer['PROPERTIES'][$arParams['OFFER_ADD_PICT_PROP']]['PROPERTY_TYPE'])
			{
				...
				{
					$arPropValue = $arOffer['PROPERTIES'][$arParams['OFFER_ADD_PICT_PROP']]['VALUE'];
...
{
	if ('' != $arParams['ADD_PICT_PROP'] && 'MORE_PHOTO' != $arParams['ADD_PICT_PROP'])
	{
		if ('F' == $arResult['PROPERTIES'][$arParams['ADD_PICT_PROP']]['PROPERTY_TYPE'])
		{
			$arResult["MORE_PHOTO"] = array();
			if (
				isset($arResult['DISPLAY_PROPERTIES'][$arParams['ADD_PICT_PROP']]['FILE_VALUE'])
				&& !empty($arResult['DISPLAY_PROPERTIES'][$arParams['ADD_PICT_PROP']]['FILE_VALUE'])
			)
			{
				$boolOnePict = isset($arResult['DISPLAY_PROPERTIES'][$arParams['ADD_PICT_PROP']]['FILE_VALUE']['ID']);
				if ($boolOnePict)
				{
					$arOneFileValue = $arResult['DISPLAY_PROPERTIES'][$arParams['ADD_PICT_PROP']]['FILE_VALUE'];
					$arPict = array(
						'ID' => intval($arOneFileValue['ID']),
						'SRC' => $arOneFileValue['SRC'],
						'WIDTH' => intval($arOneFileValue['WIDTH']),
					...
					$arResult["MORE_PHOTO"][] = $arPict;
				}
				else
				{
					foreach ($arResult['DISPLAY_PROPERTIES'][$arParams['ADD_PICT_PROP']]['FILE_VALUE'] as $arOneFileValue)
					{
						$arPict = array(
							'ID' => intval($arOneFileValue['ID']),
							'SRC' => $arOneFileValue['SRC'],
			...
			{
				$arPropValue = $arResult['PROPERTIES'][$arParams['ADD_PICT_PROP']]['VALUE'];
				if (!is_array($arPropValue))
					$arPropValue = array($arPropValue);
				foreach ($arPropValue as &$strOneValue)
				{
	...
	if (!empty($arResult["MORE_PHOTO"]))
	{
		...
		{
			$arNewMorePhoto = array(
				0 => $arResult['DETAIL_PICTURE']
			);
			foreach ($arResult["MORE_PHOTO"] as $arOnePhoto)
			{
				$arNewMorePhoto[] = $arOnePhoto;
			}
			$arResult["MORE_PHOTO"] = $arNewMorePhoto;
			unset($arNewMorePhoto);
		}
		else
		{
			$arResult['DETAIL_PICTURE'] = current($arResult["MORE_PHOTO"]);
	...
	else
	{
		if (empty($arResult['DETAIL_PICTURE']))
			$arResult['DETAIL_PICTURE'] = $arEmptyPreview;
	}
}
...
//vong 
$arResult['PACK'] = CFile::ResizeImageGet($arResult['PREVIEW_PICTURE']["ID"], array("width"=>380,"height"=>200));
$arResult['CARCASE'] = CFile::ResizeImageGet($arResult["PROPERTIES"]["CARCASE"]["VALUE"],array("width"=>380,"height"=>200));
$arResult['CERTIFICATE'] = CFile::GetPath($arResult["PROPERTIES"]["CERTIFICATE"]["VALUE"]);
$arResult['type_price_user'] = 'base';

//echo '<pre>'; var_dump($arResult ); echo '</pre>';
//echo $arResult['PROPERTIES']['SUB_ADD_BASKET']['DESCRIPTION'],'< <br>';
?>