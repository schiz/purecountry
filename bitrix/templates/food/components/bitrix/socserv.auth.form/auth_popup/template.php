...
<div class="bx-auth">
	<form method="post" name="bx_auth_services<?=$arParams["SUFFIX"]?>" target="_top" action="<?=$arParams["AUTH_URL"]?>">
		...
		<?endif;?>
		<div class="bx-auth-service-form" id="bx_auth_serv<?=$arParams["SUFFIX"]?>" style="display:block;">
			<?foreach($arAuthServices as $service)://echo '^<pre>';var_dump($service); echo '</pre>$';?>
				<?if(($arParams["~FOR_SPLIT"] != 'Y') || (!is_array($service["FORM_HTML"]))):?>
					<div id="bx_auth_serv_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>" style="display:block;"><?=str_ireplace('%26ajax%3DY', '', $service["FORM_HTML"])?></div>
				<?endif;?>
			<?endforeach?>
		</div>
